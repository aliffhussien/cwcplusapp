...
export function usePlanner() {
    const [plan, setPlan] = useState<Record<string, Record<string, string | number>>>({});
    ...
    const updatePlan = (dateStr: string, type: string, recipeId: string | number) => {
        setPlan(prev => {
            const newPlan = { ...prev };
    ...
    const autoGenerate = (weekDates: string[], recipes: Recipe[]) => {
    ...
    const clearPlan = (dateStr: string, type: string) => {
        setPlan(prev => {
            const newPlan = { ...prev };
            if (newPlan[dateStr]) {
